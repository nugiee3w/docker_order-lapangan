...
class CreateSampleOrders extends Command
{
    ...
    protected $description = 'Create 10 sample order records';

    public function handle()
    {
        $this->info('Creating 10 sample orders...');
        ...
        foreach ($orders as $orderData) {
            try {
                ...
                $created++;
                $this->line("✓ Created order: {$orderData['order_number']} - {$orderData['customer_name']}");
            } catch (\Exception $e) {
                $this->error("✗ Failed to create order: {$orderData['order_number']} - {$e->getMessage()}");
            }
        }

        $this->info("\n🎉 Successfully created {$created} sample orders!");
        
        if ($created > 0) {
            $this->comment("\nYou can now view the orders at: /orders");
            $this->comment("Sample data includes:");
            $this->comment("- Various customer names and contact info");
            $this->comment("- Different lapangan IDs (10, 11, 12)");
            $this->comment("- Mixed booking statuses (confirmed, pending, cancelled)");
            $this->comment("- Different payment statuses (paid, unpaid)");
            $this->comment("- Booking dates spread over the next 10 days");
        }
    }
}
